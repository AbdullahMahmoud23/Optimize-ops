...
function Sidebar() {
    ...
    switch (role) {
      case "admin":
        return [
          ...
          { to: "/admin/supervisors", label: "Managers", icon: "M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" },
        ];
      case "supervisor":
        return [
      ...
      case "technician":
        return [
          ...
          { to: "/technician/assignments", label: "All Recordings", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" },
  ...
  className={`lg:hidden fixed top-4 left-4 z-60 p-3 rounded-full bg-indigo-600 text-white shadow-xl border border-indigo-700/30 focus:ring-2 focus:ring-indigo-400 transition-opacity ${
        ...
        {menuItems.map((item) => (
          <li key={item.to}>
            <NavLink
              to={item.to}
              className={({ isActive }) =>
                `group flex items-center gap-3 p-3 rounded-xl transition-transform duration-150 transform focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  isActive
                    ? 'bg-gradient-to-r from-indigo-600 to-cyan-500 text-white shadow-lg scale-[1.01] border-transparent'
                    : 'bg-white text-gray-700 hover:bg-gradient-to-r hover:from-indigo-50 hover:to-cyan-50 hover:text-indigo-700 hover:shadow-sm border border-transparent hover:border-gray-100'
                }`
              }
            >
              <span className="flex items-center justify-center w-10 h-10 rounded-lg transition-colors duration-150">
                <svg
